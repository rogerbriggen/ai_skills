...
5. Parse the JSON response and display in a readable format:
...
3. Verify that `acli` is installed by running `acli --version`. If it is not found,
   ...
   ```
4. Run the following command and capture its output:
   ```bash
   acli jira workitem view --id PROJECT-123
   ```
5. Display the output as returned by `acli`, which includes the issue key, summary,
   status, type, priority, assignee, description, and subtasks.
6. If the command fails, display the error output clearly.
